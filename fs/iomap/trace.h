...
DECLARE_EVENT_CLASS(iomap_dio_class,
	TP_PROTO(struct kiocb *iocb, struct iov_iter *iter,
		 unsigned int dio_flags, size_t done_before, ssize_t ret),
	...
	TP_printk("dev %d:%d ino 0x%lx isize 0x%llx pos 0x%llx length 0x%zx "
		  "done_before 0x%zx flags %s dio_flags %s aio %d ret %zd",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->ino,
		  __entry->isize,
		  __entry->pos,
...
DEFINE_EVENT(iomap_dio_class, name,					\
	TP_PROTO(struct kiocb *iocb, struct iov_iter *iter,		\
		 ...
		 ssize_t ret),						\
	TP_ARGS(iocb, iter, dio_flags, done_before, ret))
DEFINE_DIO_RW_EVENT(iomap_dio_rw_queued);
...
TRACE_EVENT(iomap_dio_complete,
	...
	TP_printk("dev %d:%d ino 0x%lx isize 0x%llx pos 0x%llx flags %s aio %d error %d ret %zd",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->ino,
		  __entry->isize,
		  __entry->pos,
